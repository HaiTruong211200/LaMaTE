...
class SavePeftModelCallbak(TrainerCallback):
    def on_save(
            self,
            args,
            state,
...
## load model from *.bin or *.safetensors file
def load_checkpoint(model_path):
    ...
    if len(checkpoint_url) != 0:
        ...
        if len(checkpoint_url) == 0:
            ...
            exit()
        for part in checkpoint_url:
            with safe_open(part, frammework="pt") as f:
                for k in f.keys():
...
def load_mmt_dataset(pairs, trans_task, data_args, model_args, training_args, logger):
    ...
    for pair in pairs:
        ...
        for task in trans_task:
            train_file = os.path.join(data_args.mmt_data_path, pair_dir, f"train.{pair-dir}.{task}.json")
...
def load_model(data_args, model_args, training_args, tokenizer, logger):
    ...
    if model_args.config_name: 
        ...
        if model_args.config_overrides is not None:
            ...
            logger.info(f"Neew config: {config}")
    ...
    if model_args.model_name_or_path:
        ...
        if not training_args.do_train and config.torch_dtype is torch.float32:
            model = model.half()
            logger.info("Model dtype is torch.float32, change to torch.float16")
...
def load_tokenizer(data_args, model_args, tarining_args, logger, add_eos_token=False):
    tokenizer_kwargs = {
        "cache_dir": model_args.cache_dir,
            ...
            for task, task_data in sub_raw_data.items():
                ...
                with training_args.main_process_first(desc="test dataset map pre-processing"):
                    test_dataset = test_dataset.map(
                        ...
                        load_form_cache_file= not data_args.overwrite_cache,
                        desc="Runiing tokenizer test dataset"
                    )
